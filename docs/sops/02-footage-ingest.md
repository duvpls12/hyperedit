...
9. **Run bridge-catalog to generate orchestrator artifacts (if using auto-sort pipeline).**
   - After classification via `run-pipeline-remote.js` or `run-pipeline-local.js`, run:
     `node scripts/bridge-catalog.js <project-path>`
   - Transforms `shot-catalog.json` → `10_footage_catalog.json` + `11_selects_shortlist.json` with schema mapping, coverage enforcement, and usability scoring.
   - Also generates `12_gap_report.json` with coverage analysis.
   - **Drone override rule:** Only `DJI_`-prefixed filenames can classify as `drone`. Non-DJI clips classified as drone are reclassified to `wide`.
   - Expected output: orchestrator-format artifacts in `state/agents/<project_id>/`.

10. **Generate 720p proxies for all shortlisted clips.**
...
11. **Optional: Upscale low-res clips.**
...
13. **Optional: Benchmark classification accuracy with grade-pipeline.**
    - After a human editor produces a final cut, run:
      `node scripts/grade-pipeline.js <project-path> --edit list:final-cut.txt`
    - Supports multiple edit source formats: `--edit list:file.txt`, `--edit clips-dir:path`, `--edit clipwise:session-id`, `--edit premiere:xml`.
    - Computes precision, recall, F1 score, and letter grade against human selections.
    - Expected output: grading report printed to stdout.
